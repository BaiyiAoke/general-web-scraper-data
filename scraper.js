...
// 引入提取器
const { extractAnugaData,extractMessefrankfurtData,extractEurobikeData } = require('./extractors');
...
(async () => {
    ...
    const tasks = routes.map((url, idx) => limit(async () => {
        ...
        if(urlCounter >= 0) {
            ...
            try {
                ...
                // --- 提取数据 ---
                const data = await extractAnugaData(page);
                ...
                fs.writeFileSync('./output_json/excelData_anuga.json', JSON.stringify(results, null, 2), 'utf-8'); // 每次采集后保存
                console.log(`✅ Extracted: ${data.name}`);

            } catch (err) {
                console.error(`❌ Failed to process ${url}:`, err.message);
                failedUrls.push(url); // 将失败的URL记录下来
                fs.writeFileSync('./output_json/failed_routes_anuga.json', JSON.stringify(failedUrls, null, 2));
            } finally {
    ...
    if (results.length > 0) {
        ...
        // 写入文件
        workbook.xlsx.writeFile('./output_excel/anuga_contact-info.xlsx')
            .then(() => {
                console.log(`\n📦 Successfully exported ${results.length} records to anuga_contact-info.xlsx`);
            })
            .catch(err => {
    ...
    if (failedUrls.length > 0) {
        ...
        console.log('Failed URLs have been saved to failed_routes.json');
    }
})();